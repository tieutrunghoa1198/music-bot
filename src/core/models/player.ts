...
export class Player implements AbstractPlayer{
    ...
    public async replaceMessage() {
        ...
        try {
            ...
            fetchedMessage.delete().catch((err: any) => {
                console.log(err)
            })
        } catch (error) {
    ...
    private voiceStateChange() {
        this.voiceConnection.on('stateChange', async (_: VoiceConnectionState, newState: VoiceConnectionState) => {
            if (_.status === VoiceConnectionStatus.Ready && newState.status === VoiceConnectionStatus.Connecting) {
    ...
    private async startAudio(audioPlayer: AudioPlayer, songUrl: string): Promise<void> {
        const source = await play.stream(songUrl);
        const audioResource = createAudioResource(source.stream, {
            inputType: source.type
        });
