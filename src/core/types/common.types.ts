 ...
 */
export type ObjectValuesPicky<T, K extends keyof T> = T[keyof Partial<Pick<T, K>>];
...
export type GetReadonlyArrayObjectType<T> = T extends ReadonlyArray<infer U> ? U : never;
//  Return the type of NON-EMPTY items in a constant ReadonlyArray
export type GetReadonlyArrayNonEmptyObjectType<T> = T extends ReadonlyArrayNonEmpty<(infer U & { 0: infer X })> ? U : never;
