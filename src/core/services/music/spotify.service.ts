...
export class SpotifyService {
  public static async getTrack(url: string) {
    const track = (await play.spotify(url)) as SpotifyTrack;
    const searchString = track.name + ' ' + track.artists[0].name;
    const ytTracks = await play.search(searchString, {
  ...
  public static async getTrackByYT(input: string): Promise<Song> {
    return await play
      .search(input, { source: { youtube: 'video' } })
      .then((tracks: YouTubeVideo[]) => {
        console.log(
          `[PROCESS]: Get ${tracks[0].title} from ${Platform.YOUTUBE}...`,
        );
        return {
  ...
  private static async getTrackBySC(input: string): Promise<Song> {
    return await play
      .search(input, { source: { soundcloud: 'tracks' } })
      .then((tracks: SoundCloudTrack[]) => {
        console.log(
          `[PROCESS]: Get ${tracks[0].name} from ${Platform.SOUND_CLOUD}...`,
        );
        return {
